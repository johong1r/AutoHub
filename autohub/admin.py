...
class CarsAdmin(admin.ModelAdmin):
    list_display = ("id", "name", "brand", "model", "price", "year", "body", "sale", "rent", "image_preview")
    list_filter = ("brand", "body", "transmission", "year", "sale", "rent")
    search_fields = ("name", "model", "color")
    readonly_fields = ("image_preview", "join_date") 
    list_display_links = ("id", "name")

    fieldsets = (
        ("Основная информация", {
            "fields": ("name", "brand", "model", "year", "price")
        }),
        ("Характеристики", {
    ...
    def image_preview(self, obj):
        if obj.picture:
            return mark_safe(f'<img src="{obj.picture.url}" width="120" style="border-radius:10px;">')
        return "Нет фото"
